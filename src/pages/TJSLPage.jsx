...
const TJSLProfile = ({ quickFacts }) => (
  <div className="container mx-auto px-8 lg:px-16 py-20 bg-white">
    ...
    {/* Grid: allow highlighted fact to span */}
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {quickFacts.map((fact, index) => {
        const isHighlight = fact.highlight;
        const containerClasses = `${fact.bgColor} p-6 rounded-xl hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1`;
        // if highlighted, make it span more columns and increase padding/typography
        const highlightClasses = isHighlight ? 'md:col-span-2 lg:col-span-2 p-8' : '';
        return (
          <div key={index} className={`${containerClasses} ${highlightClasses}`}>
            {isHighlight ? (
              // Larger layout for highlight — two-column inside card for better visual weight
              <div className="flex flex-col lg:flex-row items-center lg:items-start gap-6">
                <div className="p-3 bg-white shadow-sm rounded-lg flex-shrink-0">
                  {/* bigger icon for highlight */}
                  {React.cloneElement(fact.icon, { className: 'w-10 h-10 text-orange-600' })}
                </div>
                <div className="flex-1">
                  <h4 className="font-medium text-gray-700 text-lg">{fact.title}</h4>
                  <p className="text-4xl md:text-5xl font-extrabold text-gray-900 leading-tight mt-2 break-words">{fact.value}</p>
                  <p className="text-sm text-gray-500 mt-2">Penerima manfaat terhitung sejak program berjalan</p>
                </div>
              </div>
            ) : (
